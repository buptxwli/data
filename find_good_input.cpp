...
I import[maxnum];
/* edit by nyhuang, memeda*/
int unit;
/* edit by nyhuang, memeda */
int tmp_t[maxnum];
bool flag_deadlock;
bool flag_end;
int step_contain[maxnum];
int cnt,all,total_t;
void graph_input()
{
    graph_size=0;
...
void debug_path(int t)
{
	for(int i=0;i<num;i++)
	{
		for(int m=0;m<=t;m++)
		{
			if(now[i][m]==endpoint[i])
            {
                finish_time[i+cnt*unit]=m;
                break;
            }
		}
	}
...
void print2()
{
    ...
    {
        if(i<unit)
            printf("%d %d\n",step_contain[i]-1,finish_time[i]);
        else
        {
            int lab=i/unit;
            int t=0;
            for(int j=0;j<lab;j++)
                t=t+tmp_t[j];
            printf("%d %d\n",step_contain[i]-1,finish_time[i]+t);
        }
    }
    return;
}
...
void crossing_token(int t)  //new
{
    if(t!=0)
    {
        if(type==1)
            ...
            end_big_release(t);
        go_release(t);
        flag_deadlock=judge_deadlock(t);
    }
    if(type==1)
        judge_small_go(t);
    else
        judge_big_go(t);
    flag_end=judge_end(t);
    if(flag_end==false &&flag_deadlock==false)
    {
        for(int k=0;k<crossnum;k++)
        {
            if(type==1)
...
int main()
{
    ...
    while (scanf("%d%d",&row,&line)!=EOF)
    {
        ...
        for(int i=0;i<num;i++)
        {
            startpoint[i].x=import[i].a;
            startpoint[i].y=import[i].b;
            endpoint[i].x=import[i].c;
            endpoint[i].y=import[i].d;
        ...
        if(flag_end==true)
        {
            ...
            prinf("%d\n",num);
            for(int i=0;i<n;i++)
                prinf("%d %d %d %d\n",startpoint[i].x,startpoint[i].y,endpoint[i].x,endpoint[i].y);
        }
		else
        {
            printf("wrong data\n");
